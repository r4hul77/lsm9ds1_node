...
struct deviceSettings
{
    ...
    std::string i2cDev;    // pointer to an instance of I2C interface

	int beginTransmission(uint8_t address){
		
		int file = open(i2cDev.c_str(), O_RDWR);
	...
	void closeTransmission(int file){
		close(file);
	}

	int write(int file, uint8_t reg, uint8_t data){
		return i2c_smbus_write_byte_data(file, reg, data);
	}

	uint8_t readByte(int file, uint8_t reg){
		return i2c_smbus_read_byte_data(file, reg);
	}

	uint8_t readBytes(int file, uint8_t reg, uint8_t* dest, uint8_t length){
		i2c_smbus_read_i2c_block_data(file, reg, length, dest);
	}
