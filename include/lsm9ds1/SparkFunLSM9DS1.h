...
public:
	...
	uint16_t begin(std::string file_loc, uint8_t agAddress = LSM9DS1_AG_ADDR(1), uint8_t mAddress = LSM9DS1_M_ADDR(1)); //By default use the default I2C addres, and use Wire port
	uint16_t beginSPI(uint8_t ag_CS_pin, uint8_t m_CS_pin);

	void calibrate(std::vector<double>& orientation, bool autoCalc = true);
	void calibrate(bool autoCalc = true);
	void calibrateMag(bool loadIn = true);
	void magOffset(uint8_t axis, int16_t offset);
	

	void set_node(rclcpp::Node*);
