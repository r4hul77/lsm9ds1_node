...
uint16_t LSM9DS1::begin(std::string dev, uint8_t agAddress, uint8_t mAddress)
{
	...
	if (whoAmICombined != ((WHO_AM_I_AG_RSP << 8) | WHO_AM_I_M_RSP))
		{
			RCLCPP_ERROR(mp_node->get_logger(), "[LSM9D1::init] failed errno %d", errno);
			return 0;
		}	
...
void LSM9DS1::calibrate(std::vector<double>& orientation, bool autoCalc)
{
	uint8_t samples = 0;
	int ii;
	...
	{	// Read the gyro data stored in the FIFO
		...
		gBiasRawTemp[2] += gz;
		readAccel();
		aBiasRawTemp[0] += ax + (int16_t)orientation[0]*(1./aRes);
		aBiasRawTemp[1] += ay + (int16_t)orientation[1]*(1./aRes); //Assuming the sensor it facing sideways
		//aBiasRawTemp[2] += az  // Assumes sensor facing up!
		aBiasRawTemp[2] += az + (int16_t)orientation[2]*(1./aRes);;
	}  
	for (ii = 0; ii < 3; ii++)
	{
...
void LSM9DS1::calibrate(bool autoCalc)
{
	...
	{	// Read the gyro data stored in the FIFO
		...
		readAccel();
		aBiasRawTemp[0] += ax;
		aBiasRawTemp[1] += ay - (int16_t)(1./aRes); //Assuming the sensor it facing sideways
		//aBiasRawTemp[2] += az  // Assumes sensor facing up!
		aBiasRawTemp[2] += az;
	}  
	for (ii = 0; ii < 3; ii++)
...
void LSM9DS1::set_node(rclcpp::Node* ros_node){
	this->mp_node = ros_node;
}
...
void LSM9DS1::readAccel()
{
	...
	if ( xgReadBytes(OUT_X_L_XL, temp, 6) == 6 ) // Read 6 bytes, beginning at OUT_X_L_XL
	{
		...
		//RCLCPP_INFO(mp_node->get_logger(), "ax : %d, ay : %d, az : %d", ax, ay, az);
		if (_autoCalc)
		{
			ax -= aBiasRaw[X_AXIS];
			ay -= aBiasRaw[Y_AXIS];
...
void LSM9DS1::readMag()
{
	...
	RCLCPP_INFO(mp_node->get_logger(), "mx : %d; my : %d; mz : %d", mx, my, mz);
}
...
float LSM9DS1::calcAccel(int16_t accel)
{
	...
	//RCLCPP_INFO(mp_node->get_logger(), "calcAccel : accel : %d", accel);
	return aRes * accel;
}
...
float LSM9DS1::calcMag(int16_t mag)
{
	...
	RCLCPP_INFO(mp_node->get_logger(), "Cal Mag register : %d and calc Val : %f", mag, mRes*mag);
	return mRes * mag;
}
...
{	
	int file = settings.device.beginTransmission(address);
	if(file > 0){
		settings.device.write(file, subAddress, data);
	}
	settings.device.closeTransmission(file);
}
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
	...
	int file = settings.device.beginTransmission(address);                 // Fill Rx buffer with result
	//RCLCPP_INFO(mp_node->get_logger(), "reading file Errono : %d; Address : %x; sub address : %x; file : %d", errno, address, subAddress, file);
	data = settings.device.readByte(file, subAddress);
	//RCLCPP_INFO(mp_node->get_logger(), "after reading reading file Errono : %d", errno);
	settings.device.closeTransmission(file);
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	uint8_t retVal;
	int file = settings.device.beginTransmission(address);
	settings.device.readBytes(file, subAddress, dest, count);
	settings.device.closeTransmission(file);
	return count;
}
